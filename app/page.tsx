...
export default function LandingPage() {
  ...
  const getProfileLabel = () => {
    ...
    if (currentProfile.gender) parts.push(currentProfile.gender === "female" ? "Sister" : currentProfile.gender === "male" ? "Brother" : "")
    ...
    <div className="relative flex h-full w-full flex-col overflow-x-hidden max-w-md mx-auto shadow-2xl bg-[#1a237e]">
      ...
      {isLoading && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-[#1a237e]/95 backdrop-blur-sm text-white p-6 text-center">
          ...
          <p className="text-sm text-blue-200 animate-pulse">{loadingStep || "Preparing your devotional..."}</p>
        </div>
      )}
      ...
      <div className="relative z-10 flex flex-1 flex-col px-6 pt-8 pb-24 gap-6">
        ...
        <div className="flex flex-col items-center">
          <div className="relative w-32 h-32 rounded-full overflow-hidden shadow-2xl shadow-blue-900/50 border-2 border-amber-400/30 bg-white/10 backdrop-blur-sm flex items-center justify-center">
            <div className="text-5xl">📖</div>
          </div>
          <h1 className="mt-4 text-2xl font-bold tracking-tight text-white text-center">Scripture for Life Stages</h1>
          <p className="mt-1 text-sm text-blue-200/80">Standard Works that speak to where you are</p>
          {isTrialActive && (
        ...
        <div className="flex flex-col">
          <div className="flex flex-col items-center text-center text-white mb-6">
            ...
            <p className="mt-1 max-w-xs text-sm text-blue-100/70">Discover scriptures personalized for your journey in the gospel</p>
            <button
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3 pt-2">
              <p className="text-sm font-semibold text-amber-200 text-center mb-1">Scripture of the Day</p>
              
              {/* Come Follow Me - Featured */}
              <button
                onClick={() => handleGenerateDevotional("ComeFollowMe")}
                disabled={isLoading}
              ...
              >
                ...
                <div className="flex-1 text-left">
                  <span className="text-sm font-bold text-white">Come, Follow Me</span>
                  <p className="text-xs text-blue-200/70">Current curriculum</p>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <button
                  onClick={() => handleGenerateDevotional("BookOfMormon")}
                  disabled={isLoading}
                ...
                >
                  ...
                  <span className="text-[10px] font-medium text-blue-100 text-center leading-tight">Book of Mormon</span>
                </button>
                
                <button
                  onClick={() => handleGenerateDevotional("DoctrineCovenants")}
                  disabled={isLoading}
                  className="flex flex-col items-center justify-center gap-2 rounded-xl bg-white/10 backdrop-blur-sm p-3 text-white transition hover:bg-white/20 active:scale-95 cursor-pointer disabled:opacity-50 border border-white/10"
                >
                  ...
                  <span className="text-[10px] font-medium text-blue-100 text-center leading-tight">D&C</span>
                </button>
                
                <button
                  onClick={() => handleGenerateDevotional("Bible")}
                  disabled={isLoading}
                  className="flex flex-col items-center justify-center gap-2 rounded-xl bg-white/10 backdrop-blur-sm p-3 text-white transition hover:bg-white/20 active:scale-95 cursor-pointer disabled:opacity-50 border border-white/10"
                >
                  ...
                  <span className="text-[10px] font-medium text-blue-100 text-center leading-tight">Bible (KJV)</span>
                </button>
              </div>
              
              <p className="mt-2 text-center text-[11px] text-blue-200/50">Personalized to your life stage in the gospel</p>
            </div>
          </div>
        </div>
      </div>
